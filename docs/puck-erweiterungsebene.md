...
- **Externe Datenquellen (allgemein):** Pucks `external`-Feld (Kopie) + `resolveData` (Frische) + Adaptor ist das offizielle Muster, FALLS je strukturierte Fremddaten in Bausteine sollen (z.B. eine Produktliste aus einer Datei). **Nicht** für ein Content-CMS — Inhalt (Text/Bilder) läuft direkt über Puck-Baustein-Props, kein separates CMS (Decap ist gestrichen, s. zielbuild-und-stand.md „Architektur-Korrektur").
